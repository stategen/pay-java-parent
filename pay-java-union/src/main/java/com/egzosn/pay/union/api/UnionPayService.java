...
public class UnionPayService extends BasePayService<IUnionPayConfigStorage> {
    ...
    public UnionPayService setPayConfigStorage(IUnionPayConfigStorage payConfigStorage) {
        ...
        try {
            certDescriptor.initPrivateSignCert(payConfigStorage.getKeyPrivateCertInputStream(), payConfigStorage.getKeyPrivateCertPwd(),
                    "PKCS12");
    ...
    private Map<String, Object> getCommonParam() {
        ...
        params.put(SDKConstants.param_accessType, payConfigStorage.getAccessType());
        return params;
    }
    ...
    public boolean signVerify(Map<String, Object> params, String sign) {
        ...
        case RSA:
            ...
            return RSA.verify(data, sign,
                    verifyCertificate(genCertificateByStr((String) params.get(SDKConstants.param_signPubKeyCert))).getPublicKey(),
                    payConfigStorage.getInputCharset());
        case RSA2:
            data = SignUtils.SHA256.createSign(data, "", payConfigStorage.getInputCharset());
            return RSA2.verify(data, sign,
                    verifyCertificate(genCertificateByStr((String) params.get(SDKConstants.param_signPubKeyCert))).getPublicKey(),
                    payConfigStorage.getInputCharset());
        case SHA1:
        case SHA256:
    ...
    /**
     * 订单超时时间。 超过此时间后，除网银交易外，其他交易银联系统会拒绝受理，提示超时。 跳转银行网银交易如果超时后交易成功，会自动退款，大约5个工作日金额返还到持卡人账户。 此时间建议取支付时的北京时间加15分钟。
    ...
    private Map<String, Object> setSign(Map<String, Object> parameters) {
        ...
        case RSA:
            ...
            signStr = SignUtils.SHA1.createSign(SignUtils.parameterText(parameters, "&", "signature"), "",
                    payConfigStorage.getInputCharset());
            parameters.put(SDKConstants.param_signature, RSA.sign(signStr,
                    certDescriptor.getSignCertPrivateKey(payConfigStorage.getKeyPrivateCertPwd()), payConfigStorage.getInputCharset()));
            break;
        case RSA2:
            parameters.put(SDKConstants.param_signMethod, SDKConstants.SIGNMETHOD_RSA);
            parameters.put(SDKConstants.param_certId, certDescriptor.getSignCertId());
            signStr = SignUtils.SHA256.createSign(SignUtils.parameterText(parameters, "&", "signature"), "",
                    payConfigStorage.getInputCharset());
            parameters.put(SDKConstants.param_signature, RSA2.sign(signStr,
                    certDescriptor.getSignCertPrivateKey(payConfigStorage.getKeyPrivateCertPwd()), payConfigStorage.getInputCharset()));
            break;
        case SHA1:
        case SHA256:
    ...
    public String toPay(PayOrder order) {
        ...
        } else if (UnionTransactionType.WEB != order.getTransactionType() && UnionTransactionType.WAP != order.getTransactionType()
                && UnionTransactionType.B2B != order.getTransactionType()) {
    ...
    public String getQrPay(PayOrder order) {
        ...
        if (this.verify(response)) {
            ...
            throw new PayErrorException(new PayException((String) response.get(SDKConstants.param_respCode),
                    (String) response.get(SDKConstants.param_respMsg), response.toJSONString()));
        }
    ...
    public String buildRequest(Map<String, Object> orderInfo, MethodType method) {
        ...
        PayConfigStorage payConfigStorage = getPayConfigStorage();
        sf.append("<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=" + payConfigStorage.getInputCharset()
                + "\"/></head><body>");
    ...
    /**
     * 功能：将订单信息进行签名并提交请求 业务范围：手机支付控件（含安卓Pay）
     * 
     * @param order
     *            订单信息
    ...
    public Map<String, Object> query(String tradeNo, String outTradeNo) {
        ...
        if (this.verify(response)) {
            ...
            throw new PayErrorException(new PayException(response.getString(SDKConstants.param_respCode),
                    response.getString(SDKConstants.param_respMsg), response.toJSONString()));
        }
        throw new PayErrorException(new PayException("failure", "验证签名失败", response.toJSONString()));
     ...
     *            退款金额
     * @param type
     *            UnionTransactionType.REFUND 或者UnionTransactionType.CONSUME_UNDO
     * @return 返回支付方申请退款后的结果
     */
    public Map<String, Object> unionRefundOrConsumeUndo(
            String origQryId,
            String orderId,
            BigDecimal refundAmount,
            UnionTransactionType type) {
    ...
    public Map<String, Object> downloadbill(Date billDate, String billType) {
        ...
        if (this.verify(response)) {
            ...
            throw new PayErrorException(new PayException(response.get(SDKConstants.param_respCode).toString(),
                    response.get(SDKConstants.param_respMsg).toString(), response.toString()));
     ...
     *            支付平台订单号或者账单类型， 具体请 类型为{@link String }或者 {@link Date }，类型须强制限制，类型不对应则抛出异常{@link PayErrorException}
