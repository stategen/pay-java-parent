...
import com.egzosn.pay.demo.service.impl.PayConfigStorateProxyWithThreadLocal;
 ...
 * 发起支付入口
 *
 * @author: egan email egzosn@gmail.com date 2016/11/18 0:25
 */
@RestController
@RequestMapping("wx")
public class WxPayController {
    ...
    @Resource
    private WxPayConfigStorageProxy wxPayConfigProxy;
    ...
    public static class WxPayConfigStorageProxy extends PayConfigStorateProxyWithThreadLocal<WxPayConfigStorage> {
        
        @Override
        public Class<? extends WxPayConfigStorage> getPayConfigStorageClz() {
            return WxPayConfigStorage.class;
        }
    ...
    public void init() {
        ...
        service.setPayConfigStorage(wxPayConfigProxy.createProxy());
    }
    ...
    public String toPay(HttpServletRequest request, BigDecimal price) {
        PayOrder order = new PayOrder("订单title", "摘要", null == price ? new BigDecimal(0.01) : price,
                UUID.randomUUID().toString().replace("-", ""), WxTransactionType.MWEB);
    ...
    public byte[] toWxQrPay(BigDecimal price) throws IOException {
        ...
        ImageIO.write(service.genQrPay(new PayOrder("订单title", "摘要", null == price ? new BigDecimal(0.01) : price,
    ...
    public String getQrPay(BigDecimal price) throws IOException {
        //获取对应的支付账户操作工具（可根据账户id）
        return service.getQrPay(new PayOrder("订单title", "摘要", null == price ? new BigDecimal(0.01) : price, System.currentTimeMillis() + "",
                WxTransactionType.NATIVE));
    }
    ...
    public Map<String, Object> microPay(BigDecimal price, String authCode) {
        //获取对应的支付账户操作工具（可根据账户id）
        //条码付
        PayOrder order = new PayOrder("egan order", "egan order", null == price ? new BigDecimal(0.01) : price,
                UUID.randomUUID().toString().replace("-", ""), WxTransactionType.MICROPAY);
        //设置授权码，条码等
    ...
    public Map<String, Object> facePay(BigDecimal price, String authCode, String openid) {
        //获取对应的支付账户操作工具（可根据账户id）
        PayOrder order = new PayOrder("egan order", "egan order", null == price ? new BigDecimal(0.01) : price,
                UUID.randomUUID().toString().replace("-", ""), WxTransactionType.FACEPAY);
        //设置人脸凭证
        order.setAuthCode(authCode);
        //  用户在商户 appid下的唯一标识
        order.setOpenid(openid);
     ...
     * @param outNo
     *            商户转账订单号
     * @param wxTransferType
     *            微信转账类型， .....这里没办法了只能这样写(┬＿┬)，请见谅 {@link com.egzosn.pay.wx.bean.WxTransferType#QUERY_BANK}
    ...
    /**
     * 查询红包记录 用于商户对已发放的红包进行查询红包的具体信息，可支持普通红包和裂变包 查询红包记录API只支持查询30天内的红包订单，30天之前的红包订单请登录商户平台查询。
     *
     * @param mchBillno
     *            商户发放红包的商户订单号
     * @return 返回查询结果
